    ...
    try {
        ...
        const startupExit2 = await register.findOne({ email })
         if (startUpExist) {
            ...
            const startupCreated = register.create({name,email,registrationNo,mobno,team,field,title,description,Requirements, phase,password});
            res.status(201).json({ message: "Registration Successful",startupCreated });
            // token: await startupCreated.generateToken(),
            // userId: startupCreated._id.toString(), });
        }
